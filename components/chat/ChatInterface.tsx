...
function resumenAccion(data: RespuestaIA, cuentas: Cuenta[]): string {
  ...
  if (data.tipo === 'gasto') {
    return `Detecté un gasto:\n\n💸 -${data.moneda || 'ARS'} ${fmt(data.monto || 0)}\n📂 ${data.categoria || 'Sin categoría'}\n📝 ${data.descripcion || '—'}${cuenta ? `\n🏦 ${cuenta.nombre}` : ''}\n\n¿Confirmás?`
  }
  if (data.tipo === 'ingreso') {
    return `Detecté un ingreso:\n\n💰 +${data.moneda || 'ARS'} ${fmt(data.monto || 0)}\n📝 ${data.descripcion || '—'}${cuenta ? `\n🏦 ${cuenta.nombre}` : ''}\n\n¿Confirmás?`
  }
  if (data.tipo === 'pago_gasto_fijo') {
    return `Voy a marcar como pagado:\n\n✅ ${data.gasto_fijo_nombre}\n💰 ${fmt(data.monto || 0)}${cuenta ? `\n🏦 ${cuenta.nombre}` : ''}\n\n¿Confirmás?`
  }
  if (data.tipo === 'pago_cuota') {
    return `Voy a registrar el pago de cuota:\n\n💳 ${data.cuota_nombre}\n💰 ${fmt(data.monto || 0)}${cuenta ? `\n🏦 ${cuenta.nombre}` : ''}\n\n¿Confirmás?`
  }
  if (data.tipo === 'movimiento_ahorros') {
    return `Voy a mover plata:\n\n💸 ${fmt(data.monto || 0)} ${data.moneda || 'ARS'}\n📤 Desde: ${data.desde}\n📥 Hacia: ${data.hasta}\n\n¿Confirmás?`
  }
  if (data.tipo === 'agregar_gasto_fijo') {
    return `${data.mensaje || `¿Confirmo agregar "${data.gasto_fijo_nombre}" como gasto fijo?`}\n\n📋 ${data.gasto_fijo_nombre}${data.monto ? `\n💰 ${fmt(data.monto)} por mes` : ''}\n\n¿Confirmás?`
  }
  if (data.tipo === 'eliminar_gasto_fijo') {
    return `${data.mensaje || `¿Elimino el gasto fijo "${data.gasto_fijo_nombre}"?`}\n\n🗑️ ${data.gasto_fijo_nombre}\n\n¿Confirmás?`
  }
  return ''
}
...
export function ChatInterface() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      if (!u) return
      setUsuarioId(u.id)
      const { data: c } = await supabase.from('cuentas').select('*').eq('usuario_id', u.id).eq('activa', true)
      setCuentas(c || [])
    }
    cargar()
  }, [user])
    ...
    try {
      ...
      if (accion.tipo === 'gasto' || accion.tipo === 'ingreso') {
        await supabase.from('transacciones').insert({
          ...
          tipo: accion.tipo,
          categoria: accion.categoria || null,
          descripcion: accion.descripcion || null,
          monto: accion.monto,
          moneda: accion.moneda || 'ARS',
          fecha,
          creado_por_ia: true,
        })
        ...
        if (gf) {
          await supabase.from('gastos_fijos_pagos').upsert({
            ...
            monto_real: accion.monto || gf.monto_estimado || null,
          }, { onConflict: 'gasto_fijo_id,mes' })

          await supabase.from('transacciones').insert({
            usuario_id: usuarioId, cuenta_id: cuenta?.id || null,
            tipo: 'gasto', subtipo: 'gasto_fijo',
            categoria: 'Gasto fijo', descripcion: accion.gasto_fijo_nombre,
            monto: accion.monto || gf.monto_estimado || 0,
          ...
          agregarMensaje({ rol: 'asistente', contenido: `❌ No encontré el gasto fijo "${accion.gasto_fijo_nombre}".` })
          return
        }
      }
        ...
        if (cuota) {
          ...
          await supabase.from('transacciones').insert({
            ...
            categoria: 'Cuota', descripcion: accion.cuota_nombre,
            monto: accion.monto || cuota.monto_por_cuota,
            moneda: 'ARS', fecha, creado_por_ia: true,
          })
          ...
          if (terminada) {
            agregarMensaje({ rol: 'asistente', contenido: `✅ ¡Guardado! Y la cuota de ${accion.cuota_nombre} está completamente pagada 🎉` })
            return
          }
        } else {
          agregarMensaje({ rol: 'asistente', contenido: `❌ No encontré la cuota "${accion.cuota_nombre}".` })
          return
        }
      }
        ...
        if (ahorro) {
          ...
          await supabase.from('ahorros').update({ monto: ahorro.monto + delta, updated_at: new Date().toISOString() }).eq('id', ahorro.id)
        }
      }

      else if (accion.tipo === 'agregar_gasto_fijo') {
        if (!accion.gasto_fijo_nombre) {
          agregarMensaje({ rol: 'asistente', contenido: '❌ No entendí el nombre del gasto fijo.' })
          return
        }
        const { error } = await supabase.from('gastos_fijos').insert({
          usuario_id: usuarioId,
          nombre: accion.gasto_fijo_nombre,
          monto_estimado: accion.monto || null,
        })
        if (error) throw error
        agregarMensaje({ rol: 'asistente', contenido: `✅ Agregué "${accion.gasto_fijo_nombre}" a tus gastos fijos. Lo vas a ver en el dashboard.` })
        return
      }

      else if (accion.tipo === 'eliminar_gasto_fijo') {
        if (!accion.gasto_fijo_nombre) {
          agregarMensaje({ rol: 'asistente', contenido: '❌ No entendí qué gasto fijo eliminar.' })
          return
        }
        const { data: gf } = await supabase
          ...
          .ilike('nombre', `%${accion.gasto_fijo_nombre}%`)
          .single()

        if (!gf) {
          agregarMensaje({ rol: 'asistente', contenido: `❌ No encontré un gasto fijo llamado "${accion.gasto_fijo_nombre}".` })
          return
        }
        await supabase.from('gastos_fijos').delete().eq('id', gf.id)
        agregarMensaje({ rol: 'asistente', contenido: `✅ Eliminé "${gf.nombre}" de tus gastos fijos.` })
        return
      }
  ...
  return (
    <div className="chat-container">
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingRight: 4 }}>
        {mensajes.map(msg => (
          <div key={msg.id} style={{ display: 'flex', justifyContent: msg.rol === 'usuario' ? 'flex-end' : 'flex-start' }}>
            {msg.rol === 'usuario' ? (
              <div style={{ background: '#4F8EF7', color: '#fff', borderRadius: '18px 18px 4px 18px', padding: '10px 14px', maxWidth: '78%', fontSize: 13.5, lineHeight: 1.5, whiteSpace: 'pre-line' }}>
                {msg.contenido}
              </div>
            ) : (
              <div style={{ background: 'rgba(255,255,255,0.04)', color: '#F8FAFC', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '4px 18px 18px 18px', padding: '10px 14px', maxWidth: '78%', fontSize: 13.5, lineHeight: 1.5, whiteSpace: 'pre-line' }}>
                {msg.contenido}
                {msg.pendienteAccion && pendiente?.mensajeId === msg.id && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                    <button onClick={confirmar} style={{ flex: 1, background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', color: '#22c55e', borderRadius: 8, padding: '8px 0', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                      ✅ Confirmar
                    </button>
                    <button onClick={cancelar} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(248,250,252,0.5)', borderRadius: 8, padding: '8px 0', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
                      ❌ Cancelar
                    </button>
                  </div>
                )}
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '12px 0' }}>
        {EJEMPLOS.map(ej => (
          <button key={ej} onClick={() => enviar(ej)} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', color: 'rgba(248,250,252,0.5)', borderRadius: 99, padding: '5px 12px', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
            {ej}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          placeholder="Ej: Pagué el gimnasio, gasté 5k en el super..."
          disabled={loading}
          style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#F8FAFC', borderRadius: 12, padding: '12px 16px', fontSize: 13, outline: 'none' }}
        />
        <button
          onClick={() => enviar()}
          disabled={loading || !input.trim()}
          style={{ width: 44, height: 44, background: '#4F8EF7', color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: (loading || !input.trim()) ? 0.4 : 1 }}
        >
