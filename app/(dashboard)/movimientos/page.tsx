...
import { BuscadorMovimientos } from '@/components/movimientos/BuscadorMovimientos'
...
export default async function MovimientosPage({
  ...
  searchParams: Promise<{ cuenta?: string; tipo?: string; mes?: string; q?: string }>
}) {
  ...
  // Cuentas para el editor
  const cuentasParaEditor = (cuentas || []).map((c: any) => ({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 780, margin: '0 auto' }}>
      ...
      <div style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {/* Búsqueda */}
        <BuscadorMovimientos initialQ={sp.q || ''} />
      ...
      {txList.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: 36, margin: '0 0 10px' }}>{q ? '🔍' : '📭'}</p>
          <p style={{ fontSize: 15, fontWeight: 600, color: 'rgba(248,250,252,0.4)', margin: 0 }}>
            {q ? `Sin resultados para "${sp.q}"` : 'Sin movimientos'}
          </p>
          <p style={{ fontSize: 13, color: 'rgba(248,250,252,0.2)', margin: '6px 0 20px' }}>
            {q ? 'Probá con otra búsqueda o limpiá los filtros' : 'Registrá tu primer movimiento o usá el Chat IA'}
          </p>
          {!q && (
            <Link href="/movimientos/nuevo" style={{
          ...
          {fechasOrdenadas.map(fecha => (
            <div key={fecha}>
              <p style={{
                ...
                textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 8px', paddingLeft: 4,
              }}>
                {labelFecha(fecha)}
              </p>
              <div style={{
                ...
                {grupos[fecha].map((tx: any, i: number) => {
                  ...
                  return (
                    <div key={tx.id} style={{
                      ...
                      <EditarMovimiento
                        transaccion={{
                          id: tx.id, tipo: tx.tipo, monto: Number(tx.monto),
                          moneda: tx.moneda || 'ARS', descripcion: tx.descripcion,
                          categoria: tx.categoria, fecha: tx.fecha, cuenta_id: tx.cuenta_id,
                        }}
                        cuentas={cuentasParaEditor}
                      />
                      <EliminarMovimiento
                        transaccionId={tx.id}
